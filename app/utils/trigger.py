...
from app.auto.executor import run_job
		...
		with self.app.app_context():
			...
			for p in projects:
				if p.enable and self.scheduler.get_job(p.id) is None:
					...
					if len(cron) < 5:
						continue
					self.scheduler.add_job(func=run_job, trigger='cron', name=p.name, replace_existing=True,
											   minute=cron[0], hour=cron[1], day=cron[2], month=cron[3],
											   day_of_week=cron[4],
											   id="%s" % p.id, args=(p.id,))
				else:
	...
	def get_jobs(self):
		...
		for p in projects:
			...
			if job is not None:
				...
				# 获取该job下最后一次运行状态
				task = Task.query.filter_by(project_id=job.id).order_by(Task.build_no.desc()).first()
				if task is not None:
